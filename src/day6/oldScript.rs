...
fn _p1(_input: &str) -> usize {
    ...
    let (mut pos_x, mut pos_y) = find_guard_position(&tableau);
...
fn _p2(_input: &str) -> usize {
    ...
    let (pos_x, pos_y) = find_guard_position(&tableau);
    visited[pos_y][pos_x] = true;
...
// Clippy me proposait de faire tableau.iter().enumerate().take(height)
// Mais il y a aucun différence
fn find_guard_position(tableau:&[Vec<char>]) -> (usize, usize) {

    // Find the guards position
    for (y, ligne) in tableau.iter().enumerate() {
        for (x, &c) in ligne.iter().enumerate() {
            if c == '^' {
                return (x, y);
            }
        }
    ...
    dbg!("Not Found");
    (0,0)
    // Original - Find the guards position
    /*
    for y in 0..height {
        for x in 0..width {
            if tableau[y][x] == '^' {
...
fn go_forward_draw_path(tableau:&mut [Vec<char>], tableau_direction:&mut HashSet<((usize, usize), char)>, width:usize, height:usize, x:&mut usize, y:&mut usize) -> bool {
    let x_origin = *x;
    let y_origin = *y;
...
fn go_forward(tableau:&mut [Vec<char>], visited:&mut [Vec<bool>], width:usize, height:usize, x:&mut usize, y:&mut usize) -> bool {
    let x_origin = *x;
    let y_origin = *y;
